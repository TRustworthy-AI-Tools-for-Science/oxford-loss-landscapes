...
def main():
    ...
    try:
        ...
        print(f"   Could not load 'Maysee/tiny-imagenet' dataset: {e}. Please ensure you have access to this dataset.")
        return
    ...
    loss = loss_fn(output, torch.zeros_like(output))  # Dummy target for loss
    ...
    print("3. Calculating model hessians...")

    _, eigenvectors = get_hessian_eigenstuff(model, loss, num_eigs_returned=2, method='scipy', landscape_type='minimum')

    model_dir_1 = copy_wts_into_model(eigenvectors[0], model)
    model_dir_2 = copy_wts_into_model(eigenvectors[-1], model)
    ...
    try:
        ...
        try:
            ...
            line_losses = oll.linear_interpolation(
                model_start=model_wrapper,
                model_end=SimpleModelWrapper(model_dir_1),
                metric=loss_metric,
                distance=1,
                steps=50,
            )
            ...
            # Plot the line
            save_figure_name = "example_hessian_cnn_loss_landscape_1D.png"
            ...
            plt.ylabel('Loss')
            plt.title('1D Loss Landscape (Eigenvector Direction)')
            plt.legend()
        ...
        try:
            ...
            # Plot the plane
            X = np.linspace(-0.5, 0.5, loss_landscape.shape[0])
            Y = np.linspace(-0.5, 0.5, loss_landscape.shape[1])
            X, Y = np.meshgrid(X, Y)
            
            fig = plt.figure(figsize=(8, 6))
            save_figure_name = 'example_hessian_cnn_loss_landscape_2D.png'
            ax = fig.add_subplot(111, projection='3d')
            ax.plot_surface(X, Y, np.log10(np.maximum(loss_landscape, 1e-10)), cmap='viridis', edgecolor='none', alpha=0.8)
            ax.set_xlabel('Direction 1')
            ax.set_ylabel('Direction 2')
            ax.set_zlabel('Loss')
            ax.set_title('2D Loss Landscape (Eigenvector Directions)')
            plt.savefig(save_figure_name, dpi=150, bbox_inches='tight')
            plt.close()
